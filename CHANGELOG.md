...
- **Vault scan deferred until after all plugins have loaded.** The vault-wide tag scan now starts after Obsidian finishes loading all plugins rather than running synchronously inside the plugin's own load step. On large vaults this prevents the plugin from delaying every other plugin's startup. Tags still hide on the first decorated frame; the status-bar count may appear a moment later than before on very large vaults, which is expected and correct.

### Fixed

- **"Rename with Tag Wrangler" actually renames now.** The hand-off tried to run a Tag Wrangler command that does not exist, and had no way to say which tag it meant, so the menu item did nothing at all. It now opens Tag Wrangler's own rename dialog for the tag you picked, which is the same thing Tag Wrangler's own right-click menu does. Tag Visibility still never renames a tag itself. The bulk "Tag Wrangler" button is **removed**: renaming is a one-tag-at-a-time dialog, so a bulk hand-off was never coherent (it would stack one dialog per selected tag). If the hand-off ever fails, you now get a notice instead of silence.
- **The Tag Visibility panel shows your whole tag list on open.** The virtualized table decided how many rows to draw by measuring its own height, but it measured before the pane had been laid out, read the result as "no room for any rows", and drew only its small overscan buffer. The scrollbar was sized from the tag count rather than from the measurement, so it looked correct for the full list while six rows were painted, and only scrolling forced the rest to appear. The same gap left dead space when the sidebar was dragged taller. The table now recomputes whenever its container is actually measured, so it fills on open and refills on resize.
...
- **Your settings are never overwritten by an unreadable data.json.** If the plugin's `data.json` was corrupted or contained invalid JSON, the plugin previously could not tell that any file existed and treated the situation as a first run: it migrated settings to defaults and wrote them back, silently replacing all your custom rules, tag overrides, and reviewed marks. That is fixed. A file that can be found but not parsed is now left exactly as it is on disk; the plugin runs on built-in defaults for the session only and shows a Notice explaining why your saved settings could not be loaded.
- **Read-only mode is now visible.** When the plugin enters read-only mode (either because `data.json` is unreadable, or because it was written by a newer version of the plugin on another device and the downgrade guard is active), settings changes would silently revert after every restart with no visible explanation. Both causes now show a Notice naming the reason and the remedy, and the state banner displays a persistent indicator for as long as read-only mode holds. Settings changes still apply and repaint in-session so the UI does not appear dead.
- **Saved tag descriptions and aliases could be silently erased by a startup scan on a large vault.** The plugin stores tag descriptions and aliases you have written in a file called `tags.json`. If Obsidian had not yet finished building its file index when the startup scan ran, every file appeared to have no tags - and the plugin wrote that empty result back to disk, replacing the sidecar. On a large vault this left the Tag Visibility panel showing "No tags yet" and the status bar showing "0 tags hidden" after every restart, with all tag descriptions and aliases permanently lost. (Reviewed marks were already safe, stored separately in `data.json` since schema v10.) The startup scan now checks whether the index is actually ready before writing anything: if the index has not yet resolved, the plugin keeps the existing file on disk and retries automatically once Obsidian signals that the full index is available. A vault with no tags at all, and a completely empty vault, still scan and save normally.

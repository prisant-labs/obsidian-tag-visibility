...
- [ ] **Bulk actions.** Select several tags and confirm hide / unhide / mark reviewed operate on the selection. There is deliberately **no** bulk Tag Wrangler action: renaming is per-tag (see the next row).
- [ ] **Rename with Tag Wrangler (DA-26).** Requires Tag Wrangler installed and enabled. On a tag's row menu, choose "Rename with Tag Wrangler": **Tag Wrangler's own rename dialog must open, pre-filled with that exact tag.** Complete the rename and confirm the tag is renamed in your notes by Tag Wrangler (this is the one action that is not display-only, and it is Tag Wrangler doing it, not Tag Visibility). Cancel the dialog on another tag and confirm you get **no** error notice: cancelling is not a failure. With Tag Wrangler disabled, the menu item must not appear at all. (This hand-off silently did nothing in every release up to rc.11.)

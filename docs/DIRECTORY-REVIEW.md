...
**Status: all findings dispositioned as of 1.0.3.** The portal's re-review of 1.0.2 (2026-07-18) raised no Errors and no new findings, listing only the items recorded here as kept or deferred. A subsequent implementability pass over those remaining items (same day) asked, for each one, whether it *could* be implemented rather than kept; the answers are recorded in the disposition column, and the one item whose fix turned out to be free (DR-15) was then resolved in 1.0.3. Fixed items name the release that resolved them. The remaining advisories are expected to persist: DR-11 clears when backlog item B018 ships (itself blocked on Obsidian publishing 1.13 API typings), DR-13 clears with it, and DR-04, DR-14, and DR-16 stand by design unless the supported-version floor changes.
...
| Id | Finding | Disposition |
|----|---------|-------------|
| DR-04 | **Vault enumeration**: the plugin lists all files in the vault (`vault.getFiles`, `getMarkdownFiles`). | **As designed, with disclosure added in 1.0.2** (`5bcb5a6`). Building the vault-wide tag index is the product's core function, and there is no enumeration-free way to build it: the only alternative API (`metadataCache.getTags()`) returns bare tag counts and would delete the table's source (frontmatter vs inline) and last-indexed columns. The README's Safety contract now states exactly what the scan touches: file paths and each file's entry in Obsidian's metadata cache. Note contents are never read; the only file the plugin reads is its own `tags.json` sidecar. The display-only invariant (no note writes, no network) is enforced by a CI test on every build. |
...
| DR-11 | **`PluginSettingTab` does not implement `getSettingDefinitions()`**; `display()` is deprecated since Obsidian 1.13.0, and without the declarative API the plugin's settings do not appear in the settings search on 1.13+. | **Deferred, tracked as backlog item B018, targeted at the 1.1 cycle.** This is a structural rework of the seven-tab settings surface (eight `display()` sites), not a mechanical swap. It is also blocked on tooling: as of 2026-07-18 the latest published API typings (`obsidian@1.12.3` on npm) do not declare `getSettingDefinitions`, so a correct, type-checked implementation is not yet possible; writing it against a guessed API shape is exactly the failure mode that produced DA-26. Nothing is broken today: the plugin's floor is `minAppVersion 1.9.10`, where `display()` is fully supported. The settings-search payoff on 1.13+ is real and is why the item is tracked rather than declined. |
...
| DR-13 | **`setWarning` is deprecated**; use `setDestructive` (`settingsTab.ts:327`). | **Kept for now, bundled into B018.** `setDestructive` does not exist at the 1.9.10 floor, so calling it would crash older clients, and it is absent from the latest published typings (`obsidian@1.12.3`), so even a feature-detected call cannot be written type-safely yet. It swaps in with B018. |

### CSS lint

| Id | Finding | Disposition |
|----|---------|-------------|
| DR-14 | **`display: contents` only partially supported by Obsidian 1.7.4** (`styles.css`, two sites). | **Kept.** Implementable (both sites are plugin-owned grid layouts that could be restructured), but pointless: the plugin's floor is Obsidian 1.9.10, where `display: contents` is fully supported, and a 1.7.4 client cannot install this plugin at all, so the flagged baseline is unreachable. Restructuring working layouts for an unreachable baseline is churn with visual-regression risk and no user benefit. |
| DR-15 | **`text-decoration` feature only partially supported by Obsidian 1.7.4** (`styles.css`, the Notebook Navigator strikethrough). | **Fixed in 1.0.3.** Initially kept on the same floor argument as DR-14, then resolved by the implementability pass as the one remaining item whose fix was free: the explicit `text-decoration-thickness: 1px` line was dropped, and the default `auto` thickness renders the same line at UI font sizes. Visually identical, one warning permanently cleared. |
| DR-16 | **Avoid `!important`** (`styles.css`, four sites). | **Kept.** All four are `display: none !important` and are the hide mechanic itself, applied to surfaces the plugin does not own (tag pane rows, Properties pills, autocomplete suggestions) plus `.tc-hidden`, which exists specifically to replace inline `style.display` toggles so themes can restyle everything else. Implementable via higher selector specificity, but that fails silently the moment a theme or snippet matches with higher specificity or its own `!important`, and the failure returns as unreproducible "this tag did not hide with theme X" reports. For a display-only plugin whose one guarantee is making a row disappear, `!important` on `display: none` is the correct tool; trading that guarantee for a lint warning would make the plugin worse. |
